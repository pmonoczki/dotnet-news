...
namespace NetMatters
{
	class Program
	{
		...
		{
            int count = 0;
            foreach (FileInfo fi in FileSearcher.GetFiles(
                new DirectoryInfo(@"C:\Windows\system32"), "*.dll", SearchOption.AllDirectories))
            {
                Console.WriteLine(fi.FullName);
                ++count;
            }
            Console.WriteLine("# of files found: {0}", count);
            Console.ReadLine();
		}
	}
